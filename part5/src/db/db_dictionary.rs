...
use diesel::dsl::sql;
use diesel::{Connection, EqAll, ExpressionMethods, OptionalExtension, SqliteConnection, QueryDsl, RunQueryDsl};
...
impl DbDictionary {
    ...
    fn update_entry(&self, entry: &DbDictionaryEntry) {
        match diesel::update(dictionary::dsl::dictionary
            ...
            .set((dictionary::used_at.eq(entry.used_at), dictionary::guessed.eq_all(entry.guessed)))
            .execute(&self.conn) {
        ...
        match self.get_word_of_today(current_day) {
            Ok(result) => match result {
                ...
                Some(mut entry) => {
                    ...
                    Some(DictionaryEntry{ word: entry.word })
                }
            }
            Err(error) => {
    ...
    fn find_word(&self, text: &str) -> Option<DictionaryEntry> {
        ...
        match db_result {
            Ok(db_word) => match db_word {
                Some(entry) => Some(DictionaryEntry { word: entry.word }),
                None => None
            },
            Err(error) => {
    ...
    fn create_word(&self, word_entry: DictionaryEntry) -> Option<DictionaryEntry> {
        match self.find_word(&word_entry.word) {
            None => {
                ...
                match db_result {
                    Ok(_) => {
                        println!("Added '{}' to the database!", &new_word.word);
