...
fn test_create_word() {
    ...
    match dictionary.create_word(DictionaryEntry{ word: String::from("rusty"), guessed: false }) {
        None => assert!(false),
        Some(_) => assert!(true)
    }

    match dictionary.create_word(DictionaryEntry{ word: String::from("testy"), guessed: false }) {
        None => assert!(false),
        Some(_) => assert!(true)
    }
